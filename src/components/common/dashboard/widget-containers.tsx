    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
      <div className="px-3 py-2 border-b border-gray-100 flex items-center justify-between">
        <div className="min-w-0">
          <h3 className="font-bold text-[#0F4C75] text-xs uppercase truncate">
            {title}
          </h3>
          {subtitle && (
...
export function DashboardChartWidget({
  ...
  const computedMax = maxValue ?? Math.max(100, ...data.map((d) => d.value));
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
      <div className="px-3 py-2 border-b border-gray-100 flex items-center justify-between">
        <div className="min-w-0">
            ...
            {data.map((item) => (
              <div key={item.label} className="w-full group">
                <div className="flex justify-between items-center mb-1.5">
                  <span
                    className="text-xs font-bold text-gray-700 truncate flex-1 pr-2"
                    title={item.label}
                  >
                    {item.label}
                  </span>
                ...
                <div className="w-full bg-gray-100 rounded-full h-2 by-gray-100 overflow-hidden shadow-inner">
                  <div
                    ...
                    style={{
                      width: `${Math.min(Math.max((item.value / computedMax) * 100, 0), 100)}%`,
