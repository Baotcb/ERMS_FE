...
import { HRCoursesView } from '@/features/hr/components/training/courses/courses-view';
...
export default async function HRCoursesPage(props: { searchParams: Promise<{ tab?: string; search?: string; courseFilter?: string; }> }) {
    ...
    const [coursesResponse, feedbacksResponse] = await Promise.all([
        tab === 'courses' ? trainingServerService.getAllCourses({ search, pageSize: DROPDOWN_PAGE_SIZE }) : Promise.resolve({ items: [] }),
