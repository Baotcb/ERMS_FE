...
export default async function ResultPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!session.user || !canAccessLearningWorkspace(session.user, session.role)) redirect('/enterprise/director/dashboard');

    const { id } = await params;
    const course = await trainingServerService.getCourseDetails(id).catch(() => null);
    if (!course) notFound();

    return <CourseResultPage initialCourse={course} />;
}
