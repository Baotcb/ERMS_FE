...
import {
  getDepartments,
  type Department,
} from "@/features/hr/api/department-service";
import {
...
import {
  useCreateEmployee,
  useUpdateEmployee,
} from "@/features/hr/hooks/use-employees";
...
function getManagedRole(roles?: string[]): EmployeeFormValues["role"] {
  const matched = roles?.find((role) =>
    managedRoles.includes(role as (typeof managedRoles)[number]),
  );
...
export function EmployeeForm({
      ...
      try {
        ...
        if (
          ...
          !loadedDepartments.find(
            (department) => department.id === initialData.departmentId,
          )
        ) {
          loadedDepartments = [
            {
              ...
              departmentName:
                initialData.departmentName || "Current Department",
              departmentCode: "",
              description: "",
              managerId: null,
              managerName: null,
  ...
  const onSubmit = async (data: EmployeeFormValues) => {
    ...
    try {
      ...
      if (!data.role) {
        form.setError("role", {
          message: "Vui lòng chọn vai trò trước khi lưu",
        });
        setIsLoading(false);
        return;
      }
      ...
      const parsedDepartmentId = data.departmentId
        ? parseInt(data.departmentId, 10)
        : null;
      const departmentId = Number.isNaN(parsedDepartmentId)
        ? null
        : parsedDepartmentId;
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {isEdit && (
            <p className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
              Họ tên, email, số điện thoại và ngày vào làm là thông tin chỉ đọc
          ...
          {hasUnmanagedRoleOnEdit && (
            <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Vai trò hiện tại của tài khoản không nằm trong nhóm role quản lý
              tại màn này. Vui lòng chọn role phù hợp trước khi lưu để tránh
              thay đổi quyền ngoài ý muốn.
            </p>
          )}

          <EmployeeJobInfo isEdit={isEdit} departments={departments} />
