...
export function FeedbackTab({
  ...
  const filtered = useMemo(() => {
    ...
    return feedbacks.filter((f) => {
      ...
      const matchCourse =
        courseFilter === "all" || f.courseName === courseFilter;
  ...
  const stats = useMemo(() => {
    ...
    const avgCourse =
      filtered.reduce((s, f) => s + f.courseRating, 0) / filtered.length;
    const avgTrainer =
      filtered.reduce((s, f) => s + f.trainerRating, 0) / filtered.length;
    return {
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 mb-5">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3 text-brand-primary mb-3">
            <MessageSquare className="w-5 h-5" /> Tổng đánh giá
          </div>
        ...
        <div className="rounded-2xl border border-amber-100 bg-amber-50 p-5 shadow-sm">
          <div className="flex items-center gap-3 text-amber-700 mb-3">
            <Star className="w-5 h-5 fill-amber-400" /> Điểm TB khóa học
          </div>
        ...
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 shadow-sm">
          <div className="flex items-center gap-3 text-blue-700 mb-3">
            <TrendingUp className="w-5 h-5" /> Điểm TB giảng viên
          </div>
          <div className="flex items-center gap-2">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((f) => (
            <div
              key={f.id}
              className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:border-blue-200 transition-colors"
            >
                ...
                <div className="space-y-1">
                  ...
                  {f.employeeEmail && (
                    <p className="text-xs text-gray-500">
                      {f.employeeEmail} • {f.departmentName}
                    </p>
                  )}
                  <p className="text-sm font-medium text-brand-primary">
                    {f.courseName}{" "}
                    <span className="text-gray-400">({f.courseCode})</span>
                  </p>
                  <p className="text-xs text-gray-400">
                    Giảng viên: {f.trainerEmail}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <span>Khóa học:</span>{" "}
                    <StarDisplay rating={f.courseRating} />
                  </div>
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <span>Giảng viên:</span>{" "}
                    <StarDisplay rating={f.trainerRating} />
                  </div>
