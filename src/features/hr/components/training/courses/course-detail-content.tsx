...
function parseCourseDescription(raw: string | undefined) {
  ...
  for (const line of lines) {
    const scheduleMatch = line.match(
      /(?:\[DRAFT\] )?Lịch trình:\s*(.+?)\s*đến\s*(.+?)\.\s*Địa điểm:\s*(.+)/i,
    );
    if (scheduleMatch) {
...
export function CourseDetailContent({
  ...
  const { description, schedule, location } = parseCourseDescription(
    course.description,
  );
    ...
    <div className="space-y-5">
      ...
      {!hideHeader && (
        <div className="flex items-start gap-3">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-primary to-brand-medium flex items-center justify-center text-white shrink-0">
          ...
          <Badge
            variant="outline"
            className={`border-0 font-semibold px-2.5 py-0.5 shrink-0 ${STATUS_COLORS[course.status] || "bg-gray-100 text-gray-700"}`}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {
            label: "Giảng viên",
            value: course.trainerName || course.trainerEmail || "Chưa gán",
            icon: "👨‍🏫",
          },
          {
            label: "Thời lượng",
            value: `${course.durationMinutes || 0} phút`,
            icon: "⏱️",
          },
          {
            label: "Bài học",
      ...
      {onViewFeedback &&
        (course.status === "Public" || course.status === "Published") && (
          <Button
            variant="outline"
            className="w-full border-brand-primary text-brand-primary hover:bg-blue-50"
            onClick={() => onViewFeedback(course.courseName)}
          >
            <MessageSquare className="w-4 h-4 mr-2" /> Xem phản hồi của khóa học
            này
          </Button>
        )}
    </div>
