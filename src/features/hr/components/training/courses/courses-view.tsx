...
export function HRCoursesView({
    ...
    const updateUrl = useCallback((newTab: string, newSearch: string, newCourseFilter: string) => {
        const params = new URLSearchParams();
        if (newTab !== 'courses') params.set('tab', newTab);
        if (newSearch) params.set('search', newSearch);
        if (newCourseFilter !== 'all') params.set('courseFilter', newCourseFilter);
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                ...
                <div className="flex bg-gray-100 rounded-lg p-1 gap-1 flex-shrink-0">
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${tab === 'courses' ? 'bg-white text-[#0F4C75] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${tab === 'feedback' ? 'bg-white text-[#0F4C75] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
