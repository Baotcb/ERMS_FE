...
export function ConsolidateRequests({
  ...
  initialData?: { items: TrainingRequest[] };
}) {
  ...
  const [planName, setPlanName] = useState(
    `Kế hoạch đào tạo năm ${currentYear}`,
  );
  ...
  const departmentOptions = useMemo(
    () =>
      Array.from(new Set(pendingRequests.map((r) => r.departmentName))).sort(),
    [pendingRequests],
  );
    ...
    await execute(
      async () => {
        const res = await hrTrainingService.createPlan({
          ...
          description:
            description ||
            `Kế hoạch tổng hợp từ ${selectedIds.length} yêu cầu của các phòng ban.`,
          startDate: startDate,
          endDate: endDate,
      ...
      },
      {
        successMessage: {
          title: "Thành công",
          description:
            "Đã tạo kế hoạch đào tạo năm và cập nhật trạng thái các yêu cầu.",
        },
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Xem và chọn các yêu cầu từ phòng ban để đưa vào kế hoạch năm
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() =>
                    setSelectedIds(
                      filteredRequests.map((r: TrainingRequest) => r.id),
                    )
                  }
                >
                  Chọn tất cả
            ...
            <Table>
              <TableHeader className="bg-gray-50">
                <TableRow>
                  ...
                  <TableHead className="text-center font-bold text-[#0F4C75]">
                    Thao tác
                  </TableHead>
                </TableRow>
              </TableHeader>
                ...
                {isLoading ? (
                  ...
                  filteredRequests.map((request: TrainingRequest) => (
                    <TableRow
                      key={request.id}
                      className={
                        selectedIds.includes(request.id) ? "bg-blue-50/30" : ""
                      }
                    >
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell>
                        {request.estimatedParticipants} học viên
                      </TableCell>
                      <TableCell className="text-center">
                        <Button
                          ...
                          className="text-[#0F4C75] hover:bg-blue-50"
                        >
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2 text-xs text-amber-600 bg-amber-50 p-3 rounded-lg border border-amber-100">
                ...
                <p>
                  Các yêu cầu được chọn sẽ được chuyển sang trạng thái &quot;Đã
                  duyệt&quot; và gán vào kế hoạch này.
                </p>
              </div>
              ...
              <Button
                ...
                disabled={
                  selectedIds.length === 0 || isSubmitting || !plannedBudget
                }
