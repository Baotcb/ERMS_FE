...
export function CreateCourseDialog({
    ...
    const handleNextStep = async () => {
        const isValid = await form.trigger(['trainingPlanId', 'courseName', 'courseCode']);
        if (isValid) {
            setWizardStep(2);
        }
    };
    ...
    const onSubmit = async (data: z.infer<typeof createCourseSchema>) => {
        ...
        if (!provisionalStartTime || Number.isNaN(provisionalStartTime.getTime())) {
            toast({
                ...
                description: 'Không xác định được thời gian bắt đầu từ training plan. Vui lòng kiểm tra lại.',
                variant: 'destructive',
            });
            return;
        }
        ...
        try {
            const payload: CreateCourseCommand = {
                ...
                maxEnrollments: data.maxEnrollments ? Number(data.maxEnrollments) : undefined,
            ...
            if (userEmail && data.trainerEmail.trim().toLowerCase() !== userEmail.toLowerCase()) {
                toast({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[720px]" onInteractOutside={(e) => e.preventDefault()}>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-5 py-2">
                        {wizardStep === 1 && (
                            <>
                                {!plan && (
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="grid gap-2">
                                                ...
                                                <FormMessage />
                                                <p className="text-xs text-gray-500">Mỗi khóa học phải gắn với một training plan đã được duyệt.</p>
                                            </FormItem>
                                        )}
                                    />
                                )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="grid gap-2">
                                            ...
                                            <FormControl>
                                                <Textarea {...field} placeholder="Nhập mô tả khóa học" rows={4} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                            ...
                            <>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="grid gap-2">
                                                ...
                                                <FormMessage />
                                                <p className="text-xs text-gray-500">Nhập email giảng viên để hệ thống ghi nhận.</p>
                                            </FormItem>
                                        )}
                                    />
                                )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="grid gap-2">
                                                ...
                                                <FormControl>
                                                    <Input {...field} type="number" min="1" placeholder="Ví dụ: 120" />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="grid gap-2">
                                                ...
                                                <FormControl>
                                                    <Input {...field} type="number" min="1" placeholder="Ví dụ: 30" />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                ...
                                <div className="grid gap-2 md:grid-cols-[1fr_auto] md:items-center">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="grid gap-2">
                                                ...
                                                <FormControl>
                                                    <Input {...field} placeholder="Ví dụ: Quiz" />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        name="isMandatory"
                                        render={({ field }) => (
                                            <FormItem className="flex items-center justify-between gap-3 rounded-lg border px-4 py-3 md:min-w-[180px]">
                                                <FormLabel className="m-0 select-none cursor-pointer">Bắt buộc</FormLabel>
                                                <FormControl>
                        ...
                        <DialogFooter className="mt-4">
                            ...
                            {wizardStep === 1 ? (
                                <Button type="button" onClick={handleNextStep} className="bg-[#0F4C75] hover:bg-[#1A5F8C]">
                                    Tiếp tục
                                </Button>
                            ) : (
                                <>
                                    <Button type="button" variant="outline" onClick={() => setWizardStep(1)} disabled={form.formState.isSubmitting}>
                                        Quay lại
                                    </Button>
                                    <Button type="submit" disabled={form.formState.isSubmitting} className="bg-[#0F4C75] hover:bg-[#1A5F8C]">
