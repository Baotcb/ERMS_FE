...
import {
  workshopService,
  type WorkshopConfirmation,
} from "@/features/hr/api/workshop-service";
...
export function WorkshopManagementList() {
  ...
  const [evidenceDialog, setEvidenceDialog] = useState<{
    open: boolean;
    confirmation: WorkshopConfirmation | null;
    courseName: string;
  }>({
    open: false,
    confirmation: null,
  ...
  const { data: coursesData, mutate } = useSWR("hr-workshop-courses", () =>
  ...
  const offlineCourses = (coursesData?.items || []).filter(
    (c) => c.isOnline === false,
  );
  ...
  const { data: confirmations, mutate: mutateConfirmations } = useSWR(
    offlineCourses.length > 0
      ? ["workshop-confirmations", offlineCourses.map((c) => c.id).join(",")]
      : null,
    async () => {
      const results: Record<string, WorkshopConfirmation | null> = {};
      await Promise.all(
        offlineCourses.map(async (course) => {
          results[course.id] = await workshopService.getWorkshopConfirmation(
            course.id,
          );
        }),
      );
  ...
  const stats = useMemo(() => {
    ...
    const confirmed = offlineCourses.filter(
      (c) => !!confirmations?.[c.id],
    ).length;
    ...
    <div className="w-full max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Xác nhận hoàn thành buổi đào tạo trực tiếp (offline) để mở khóa bài
            kiểm tra cho học viên.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-3 bg-white px-4 py-3 rounded-lg border border-gray-100 shadow-sm">
        ...
        <Select
          value={statusFilter}
          onValueChange={(v) =>
            setStatusFilter(v as "all" | "confirmed" | "pending")
          }
        >
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-brand-primary">
                        {offlineCourses.length === 0
                          ? "Chưa có khóa workshop nào"
                          : "Không tìm thấy kết quả"}
                      </p>
                      <p className="text-sm text-gray-400 mt-1">
                        {offlineCourses.length === 0
                          ? "Các khóa đào tạo offline đã mở sẽ xuất hiện tại đây."
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          setEvidenceDialog((prev) => ({ ...prev, open }))
        }
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-brand-primary">
              Chi tiết xác nhận Workshop
            </DialogTitle>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-600">
                    {format(
                      new Date(evidenceDialog.confirmation.confirmedAt),
                      "dd/MM/yyyy HH:mm",
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-600">
                    {evidenceDialog.confirmation.evidencePhotoUrls.length} ảnh
                  </p>
                </div>
              </div>
              ...
              {evidenceDialog.confirmation.evidencePhotoUrls.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {evidenceDialog.confirmation.evidencePhotoUrls.map(
                      (url, i) => (
                        <a
                          key={i}
                          href={url}
                          target="_blank"
                          rel="noreferrer"
                          className="rounded-xl overflow-hidden border border-gray-200 aspect-video block hover:shadow-md transition-shadow"
                        >
                          <Image
                            src={url}
                            alt={`Minh chứng ${i + 1}`}
