...
const STATUS_CONFIG: Record<OfferStatusDisplayProps['status'], StatusConfig> = {
    ...
    rejected: {
        ...
        description: 'Cảm ơn bạn đã phản hồi. Chúc bạn tìm được cơ hội phù hợp hơn trong tương lai.',
    },
    error: {
        icon: (
    ...
    invalid: {
        ...
        description: 'Liên kết này không đúng định dạng. Vui lòng kiểm tra lại email.',
    },
}

export function OfferStatusDisplay({ status, errorMsg }: OfferStatusDisplayProps) {
    const config = STATUS_CONFIG[status]

    // Nếu là lỗi và có errorMsg cụ thể, dùng errorMsg thay cho description mặc định
    const description = status === 'error' && errorMsg ? errorMsg : config.description

    return (
        <>
            <div className="mx-auto mb-5">{config.icon}</div>
