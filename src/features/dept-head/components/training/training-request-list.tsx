...
export function TrainingRequestList({
  ...
  initialData?: TrainingRequestsResult;
}) {
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Theo dõi và quản lý các yêu cầu đào tạo của phòng ban ({totalCount}{" "}
            yêu cầu)
          </p>
        </div>
      ...
      {requests.some((r: TrainingRequest) => r.status === "AddedToPlan") && (
        <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 text-sm text-[#0F4C75]">
          ...
          <span className="flex-1">
            Một số yêu cầu của bạn đã được thêm vào kế hoạch đào tạo và được
            Giám đốc phê duyệt. Hãy vào <strong>Kế hoạch đào tạo</strong> để xem
            khóa học HR đã khởi tạo và phân công giảng viên, học viên.
          </span>
          <Button
            size="sm"
            onClick={() => router.push("/enterprise/dept-head/training/plans")}
              ...
              {isLoading ? (
                ...
                requests.map((request) => (
                  ...
                  >
                    ...
                    <TableCell>{renderUrgency(request.urgency)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`border-0 font-semibold px-2.5 py-0.5 ${STATUS_COLORS[request.status] || "bg-gray-100"}`}
                      >
