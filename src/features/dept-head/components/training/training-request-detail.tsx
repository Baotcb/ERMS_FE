...
export function TrainingRequestDetail({
  request,
  open,
  onOpenChange,
  isPlanClosed = false,
}: TrainingRequestDetailProps) {
          ...
          <DialogHeader>
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col items-end gap-2">
                <Badge
                  className={`${status.color} border font-bold px-3 py-1 shadow-sm`}
                >
                  {status.label}
                </Badge>
                {isPlanClosed && (
                  <Badge className="bg-amber-100 text-amber-800 border-amber-300 font-bold px-2 py-0.5 text-[10px]">
                    🔒 Chỉ xem
                  </Badge>
                )}
              </div>
