...
type TrainingRequestFormExtendedProps = TrainingRequestFormProps & {
...
export function TrainingRequestForm({
  ...
  initialData,
  isPlanClosed = false,
}: TrainingRequestFormExtendedProps) {
  const { execute, isSubmitting: isLoading } = useAsyncAction();
  const { toast } = useToast();
  const [detectedEmp, setDetectedEmp] = useState<{
    id: string;
    ...
    const detectUser = async () => {
      try {
        ...
        const empRes = await apiClient.get(
          `/api/Employees?search=${encodeURIComponent(email)}&pageSize=5`,
        );
        if (empRes.ok) {
        ...
        setDetectError(
          "Không thể xác định thông tin nhân viên hiện tại. Vui lòng đăng nhập lại.",
        );
      } catch (e) {
        const msg = e instanceof Error ? e.message : "Lỗi không xác định";
        setDetectError(
          `Không thể xác định thông tin nhân viên hiện tại: ${msg}`,
        );
      }
    };
      ...
      },
      {
        successMessage: {
          ...
          description: initialData
            ? "Cập nhật yêu cầu đào tạo thành công."
            : "Yêu cầu đào tạo đã được gửi đi.",
        },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden bg-white border-none shadow-2xl">
        <div className="bg-gradient-to-r from-[#0F4C75] to-[#3282B8] px-6 py-8 text-white">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              ...
              {isEditMode ? "Cập Nhật Yêu Cầu" : "Gửi Yêu Cầu Đào Tạo"}
            </DialogTitle>
        ...
        {isPlanClosed && (
          <div className="flex items-center gap-2 px-6 py-2.5 bg-amber-50 border-b border-amber-200 text-amber-800 text-xs font-semibold">
            <svg
            ...
            Kế hoạch đã đóng — Chỉ xem. Yêu cầu không thể chỉnh sửa.
          </div>
        )}
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      <FormLabel className="text-[#0F4C75] font-semibold">
                        Chủ đề đào tạo <span className="text-red-500">*</span>
                      </FormLabel>
                      <FormControl>
                        <Input
                          placeholder="Vd: Kỹ năng giao tiếp chuyên nghiệp"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      ...
                      <FormControl>
                        <Input
                          placeholder="Vd: Toàn bộ nhân viên phòng Sales"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="Nêu rõ lý do và mục tiêu cần đạt được sau khóa học..."
              ...
              <DialogFooter className="px-0 py-4 bg-transparent border-t flex items-center justify-between">
                <div className="flex items-center text-xs text-gray-500 gap-1 italic">
                  <Info className="w-3 h-3" /> Yêu cầu của bạn sẽ được gửi tới
                  HR Manager để xem xét.
                </div>
                <div className="flex gap-3">
                  ...
                  <Button
                    type="submit"
                    className="bg-[#0F4C75] hover:bg-[#1A5F8C] text-white min-w-[120px]"
                    disabled={
                      isLoading ||
                      (!detectedEmp?.id && !isEditMode) ||
                      isPlanClosed
                    }
                    title={
                      isPlanClosed
