...
function InterviewTypeBadge({ type }: { type: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={`text-xs w-full justify-center ${c.className}`}
    >
      {c.label}
    </Badge>
  );
...
export function DeptHeadInterviewList() {
  ...
  const handleDecision = (item: InterviewFeedbackSummaryDto) => {
    ...
    router.push(
      `/enterprise/dept-head/decision/${item.applicationId}?${params}`,
    );
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-[#0C4A6E]/70 text-base">
          Xem kết quả đánh giá từ người phỏng vấn và ra quyết định cho từng ứng
          viên.
        </p>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
              ...
              <h3 className="text-lg font-semibold text-slate-600">
                {decisionTab === "decided"
                  ? "Chưa có phỏng vấn nào đã ra quyết định"
                  : decisionTab === "pending"
                    ? "Chưa có phỏng vấn cần quyết định"
                    : "Chưa có dữ liệu phỏng vấn"}
              </h3>
              <p className="text-slate-400 mt-2 max-w-sm">
                {decisionTab === "decided"
                  ? "Các buổi phỏng vấn bạn đã duyệt xong sẽ được lưu trữ lịch sử tại đây."
                  : "Khi người phỏng vấn gửi đánh giá xong, danh sách sẽ hiện ở đây để bạn đưa ra quyết định cuối."}
              </p>
            </div>
          ) : (
...
function DeptHeadInterviewTable({
  data,
  onDecision,
}: DeptHeadInterviewTableProps) {
  return (
    <Table>
      <TableHeader>
            ...
            >
              ...
              <TableCell className="px-6 py-4 align-middle">
                <div className="flex flex-col gap-1 w-[90px]">
                  <Badge
                    variant="outline"
                    className="text-xs bg-slate-50 text-slate-500 border-slate-200 w-full justify-center"
                  >
