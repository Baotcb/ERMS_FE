...
export function TrainerCourseDashboard({
  initialCourse,
  teachingBasePath = "/enterprise/employee/teaching",
}: TrainerCourseDashboardProps) {
  ...
  const buildCourseUpdatePayload = (overrides: Partial<Course>) => {
    const normalizedTrainerEmail = (course.trainerEmail || "")
      .trim()
      .toLowerCase();
    ...
    if (!course.startTime) {
      throw new Error(
        "Khóa học chưa có thời gian bắt đầu. Vui lòng nhờ HR thiết lập lịch trước.",
      );
    }
  ...
  const handleSaveDraft = async () => {
    ...
    if (!values.courseName) {
      toast({
        title: "Thiếu tên khóa học",
        description: "Vui lòng nhập tên khóa học trước khi lưu.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          "Thông tin cơ bản đã lưu lên hệ thống. Nội dung học trong tab Nội dung học được tự lưu nháp local.",
      });
    } catch (error) {
      const errorMessage =
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast({
        title: "Chúc mừng!",
        description: "Khóa học đã sẵn sàng cho học viên tham gia.",
      });
      router.push(teachingBasePath);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Không thể mở khóa học.";
      toast({
        title: "Lỗi",
        description: errorMessage,
        variant: "destructive",
  ...
  const publishChecks = [
    {
      ...
      ok: Boolean(
        course.courseName && course.description && course.durationMinutes,
      ),
    },
    {
      label: "Lịch trình",
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        {/* Top color strip */}
        <div
          className={`h-1.5 ${isCourseCompleted ? "bg-gradient-to-r from-green-400 to-green-500" : "bg-gradient-to-r from-[#3282B8] to-[#0F4C75]"}`}
        />
        ...
        <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={handleBack}
              className="w-10 h-10 rounded-full hover:bg-[#BBE1FA]/30 flex items-center justify-center transition-colors"
            >
            ...
            <div>
              ...
              <h1 className="text-xl md:text-2xl font-black text-[#0F4C75] tracking-tight">
                {course.courseName}
              </h1>
            </div>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm min-h-[500px] p-8">
            ...
            <div className="max-w-3xl space-y-8">
              <div className="space-y-2">
                <h2 className="text-xl font-black text-[#0F4C75]">
                  Nhận nhiệm vụ & Khởi tạo khóa học
                </h2>
                <p className="text-gray-500 text-sm">
                  Hoàn thiện thông tin cơ bản trước khi tạo nội dung học.
                </p>
              </div>

              <Form {...form}>
                <form
                  onSubmit={form.handleSubmit(handleSaveBasics)}
                  className="space-y-6"
                >
                  <FormField
          ...
          >
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#BBE1FA]/40 bg-[#BBE1FA]/10 p-5">
                <h2 className="text-lg font-black text-[#0F4C75]">
                  Thiết kế lộ trình học tập
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                  Tạo lesson, đính kèm video và upload tài liệu để học viên có
              ...
              <div className="flex items-center justify-between pt-2">
                <Button
                  variant="ghost"
                  className="text-gray-600 font-medium gap-2"
                  onClick={() => setActiveTab("basics")}
                >
                  <ChevronLeft className="w-4 h-4" /> Quay lại
                </Button>
                <Button
                  className="bg-[#0F4C75] hover:bg-[#1B262C] text-white px-6 rounded-xl font-bold gap-2"
          ...
          >
            {isCourseCompleted && (
              <div className="flex items-center gap-2 px-4 py-3 mb-4 bg-green-50 border border-green-200 rounded-xl text-green-800 text-sm font-semibold">
                <CheckCircle2 className="w-4 h-4 shrink-0" />
                Khóa học đã hoàn thành và đã mở — Nội dung bài kiểm tra chỉ xem,
                không thể chỉnh sửa.
              </div>
            )}
            <ExamBuilder
          ...
          >
            <div className="max-w-2xl mx-auto space-y-8">
              <div className="text-center space-y-2">
                <h2 className="text-2xl font-black text-[#0F4C75]">
                  Kiểm tra & Mở khóa học
                </h2>
                <p className="text-gray-500 text-sm">
                  Hệ thống tự động kiểm tra các mục bên dưới trước khi mở khóa.
                </p>
              </div>
              ...
              <div className="space-y-3">
                {publishChecks.map((check) => (
                  ...
                  >
                    ...
                    <div>
                      <p
                        className={`font-bold text-sm ${check.ok ? "text-green-800" : "text-red-800"}`}
                      >
                        {check.label}
                      </p>
                      <p
                        className={`text-xs ${check.ok ? "text-green-600" : "text-red-600"}`}
                      >
                        {check.desc}
                      </p>
                    </div>
              ...
              >
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Sau khi mở khóa, khóa học sẽ xuất hiện trong danh mục đào
                      tạo và học viên có thể bắt đầu tham gia. Hành động này
                      không thể hoàn tác.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
