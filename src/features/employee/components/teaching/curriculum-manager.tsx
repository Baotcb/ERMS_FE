...
export function CurriculumManager({ courseId }: CurriculumManagerProps) {
    ...
    const uploadMaterialViaCloudinary = async (file: File): Promise<string> => {
        ...
        if (!data.secure_url) throw new Error('Không nhận được URL tài liệu từ Cloudinary.');
            ...
            if (message.includes('Backend chưa hỗ trợ endpoint upload tài liệu')) {
                try {
                    ...
                    try {
                        ...
                        await apiClient.put(`/api/Lessons/${lessonId}/document-url`, { documentUrl: uploadedUrl });
                    } catch {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <h3 className="text-lg font-bold text-[#0F4C75]">Chương trình học ({lessons.length} bài giảng)</h3>
                    {lastSavedAt && (
                    ...
                    <DialogContent className="rounded-3xl max-w-2xl" onInteractOutside={(e) => e.preventDefault()}>
                        <DialogHeader>
                            ...
                            <DialogDescription className="text-sm text-gray-500">
                                Điền thông tin cơ bản của bài giảng. Tài liệu có thể upload sau khi tạo.
                            </DialogDescription>
                        </DialogHeader>
            ...
            {isCurriculumUnavailable && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Backend hiện chưa mở endpoint curriculum cho khóa học này. Nội dung đang được lưu nháp local.
                </div>
            )}
                ...
                {lessons.length === 0 ? (
                    <div className="p-20 border-2 border-dashed border-gray-100 rounded-3xl text-center space-y-4">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-gray-400">Hãy bắt đầu bằng cách thêm bài giảng đầu tiên.</p>
                        </div>
                    </div>
                ) : (
