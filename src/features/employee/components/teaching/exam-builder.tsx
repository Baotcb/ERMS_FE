...
export function ExamBuilder({
  ...
  const [quizTitle, setQuizTitle] = useState(
    `Bài thi cuối khóa ${courseLabel}`,
  );
  const [description, setDescription] = useState(
    "Đánh giá cuối khóa để xác định học viên đạt hay không đạt.",
  );
  ...
  const handleDownloadTemplate = () => {
    ...
    toast({
      ...
      description:
        "Template CSV đã được tải xuống. Bạn có thể mở bằng Excel rồi điền câu hỏi theo đúng cột yêu cầu.",
    });
  };
  ...
  const handleDeleteQuiz = async () => {
    ...
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn xóa bài thi này và tạo lại không? Toàn bộ câu hỏi cũ sẽ bị xóa.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Lỗi không xác định khi xóa.";
      toast({
  ...
  const handleSave = async () => {
    if (savedQuizId) {
      toast({
        title: "Quiz đã tồn tại",
        description: "Khóa học này đã có bài thi cuối khóa, không cần tạo lại.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!quizFile) {
      toast({
        title: "Chưa chọn file CSV",
        description: "Vui lòng upload file CSV chứa bộ câu hỏi.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (
      ...
      toast({
        title: "Điểm đạt không hợp lệ",
        description: "Điểm đạt phải từ 0 đến 100.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (Number.isNaN(parsedMaxAttempts) || parsedMaxAttempts <= 0) {
      toast({
        title: "Số lượt làm không hợp lệ",
        description: "Số lượt làm tối đa phải lớn hơn 0.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Không thể tạo bài thi cuối khóa.";
      toast({
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-[#0F4C75]">
        ...
        <span>
          Hệ thống kiểm tra nội dung file trước khi tạo bài thi để đảm bảo dữ
          liệu hợp lệ. Chỉ hỗ trợ định dạng CSV.
        </span>
      </div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-100">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">
            Tạo quiz để đánh giá học viên đạt hay không đạt sau khi hoàn thành
            khóa học.
          </p>
        </div>
        ...
        <div className="flex items-center gap-6 bg-gray-50 p-4 rounded-2xl border border-gray-100">
          <div className="space-y-1">
            ...
            <Input
              ...
              disabled={Boolean(savedQuizId) || isReadOnly}
            />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card className="rounded-3xl border-dashed border-[#3282B8]/30 bg-blue-50/30 p-6 shadow-sm space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Chỉ hỗ trợ `.csv` để kiểm tra dữ liệu trước khi tạo quiz (tránh tạo
            ghost quiz).
          </p>
          <Button
            type="button"
            ...
            className="justify-start border-[#3282B8]/30 text-[#0F4C75] hover:bg-white"
            disabled={Boolean(savedQuizId) || isReadOnly}
          >
                  ...
                  <div className="text-xs text-green-700">
                    ...
                    {csvPreviewFirst && (
                      <span className="text-gray-500">
                        {" "}
                        — Câu 1: &quot;{csvPreviewFirst.slice(0, 60)}
                        {csvPreviewFirst.length > 60 ? "..." : ""}&quot;
                      </span>
                    )}
                  </div>
                )}
          ...
          <div className="flex items-start gap-2 text-xs text-gray-500">
            ...
            <span>
              Các cột theo thứ tự: QuestionText, OptionA, OptionB, OptionC,
              OptionD, CorrectAnswer, Explanation, Points, OrderIndex. Đáp án
              đúng lưu mã A/B/C/D.
            </span>
          </div>
        </Card>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-green-200 bg-green-50 px-5 py-4">
          <div className="space-y-1 text-sm text-green-800">
            ...
            <p className="opacity-90">
              Học viên sẽ tự động tham gia bài kiểm tra này theo chương trình
              học. Không cần cung cấp mã thủ công.
            </p>
          </div>
          <Button
            variant="destructive"
      ...
      <div className="flex justify-end pt-8 gap-4">
        <Button
          ...
          disabled={isSaving || Boolean(savedQuizId) || isReadOnly}
        >
          {isSaving ? (
