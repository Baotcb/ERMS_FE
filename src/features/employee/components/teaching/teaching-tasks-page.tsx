...
const STEPS = [
  {
    ...
    cta: "Thiết lập nội dung",
    icon: BookOpen,
  },
  {
    key: 2,
    label: "Tạo bài kiểm tra",
    cta: "Tạo quiz cuối khóa",
    icon: Layers,
  },
...
function getStatusStep(course: Course): number {
  if (course.status === "Public" || course.status === "Published") return 3;
    ...
    <div className="flex items-center gap-1">
      {STEPS.map(({ key }, i) => (
        <div key={key} className="flex items-center">
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`w-4 h-0.5 ${current > key ? (current === 3 ? "bg-green-300" : "bg-[#BBE1FA]") : "bg-gray-100"}`}
            />
          )}
        </div>
      ))}
...
export function TeachingTasksPage({
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "draft" | "public">(
    "all",
  );
  ...
  const stats = useMemo(() => {
    const all = initialCourses.filter((c) =>
      user ? isCourseOwnedByUser(c, user, user.role) : false,
    );
    const published = all.filter(
      (c) => c.status === "Public" || c.status === "Published",
    ).length;
    const draft = all.length - published;
    const totalEnroll = all.reduce(
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0F4C75] via-[#1B262C] to-[#0F4C75] p-8 md:p-10 text-white">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#BBE1FA]/70">
              Quản lý giảng dạy
            </p>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight">
              Xin chào, {user?.fullName?.split(" ").pop() || "Giảng viên"} 👋
            </h1>
            <p className="text-[#BBE1FA]/80 text-sm">
              Quản lý các khóa đào tạo được phân công cho bạn.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            {[
              ...
              <div
                key={label}
                className="bg-white/10 backdrop-blur rounded-2xl px-5 py-3 text-center min-w-[80px] border border-white/10"
              >
                <p className={`text-2xl font-black ${color}`}>{value}</p>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-[#BBE1FA]/60">
                  {label}
                </p>
              </div>
            ))}
        ...
        {paginatedCourses.length === 0 ? (
          <div className="col-span-full rounded-3xl border-2 border-dashed border-gray-200 bg-white p-16 text-center space-y-4">
            <div className="w-16 h-16 bg-[#BBE1FA]/30 rounded-full flex items-center justify-center mx-auto">
              {search.trim() ? (
                <Search className="w-7 h-7 text-[#3282B8]" />
              ) : (
                <GraduationCap className="w-7 h-7 text-[#3282B8]" />
              )}
            </div>
            <p className="text-lg font-bold text-[#0F4C75]">
              {search.trim()
                ? "Không tìm thấy khóa học phù hợp"
                : teachingBasePath?.includes("hr")
                  ? "Không có khóa học nào từ Giảng viên ngoài"
            ...
            <p className="text-sm text-gray-400">
              {search.trim()
                ? "Thử từ khóa khác."
                : teachingBasePath?.includes("hr")
                  ? "Hiện tại không có khóa học nào từ giảng viên bên ngoài cần bạn thiết lập thay."
                  : "Bạn sẽ nhận được thông báo khi có khóa học được phân công."}
            </p>
          </div>
        ) : (
          paginatedCourses.map((course) => {
            ...
            return (
              <div
                key={course.id}
                className="group rounded-3xl border border-gray-100 bg-white shadow-sm hover:shadow-xl hover:border-[#BBE1FA] transition-all duration-300 flex flex-col overflow-hidden"
              >
                {/* Top Strip */}
                <div
                  className={`h-1.5 ${isPublished ? "bg-gradient-to-r from-green-400 to-green-500" : "bg-gradient-to-r from-[#3282B8] to-[#0F4C75]"}`}
                />
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <Badge
                      className={`border-0 font-semibold text-[10px] px-2.5 ${
                        isPublished
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-50">
                    ...
                    >
                      <a href={`${teachingBasePath}/course/${course.id}`}>
                        ...
                        {isPublished || course.status === "Closed"
                          ? "Xem chi tiết"
                          : stepConfig.cta}
