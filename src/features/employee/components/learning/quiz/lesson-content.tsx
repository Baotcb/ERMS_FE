    ...
    return (
        <section className="space-y-5">
            <div className="learning-card p-6 space-y-4">
                ...
                {/* Video Player */}
                {activeLesson.videoUrl && <VideoPlayer url={activeLesson.videoUrl} title={activeLesson.title} />}
                ...
                <div className="flex items-center gap-2">
                    <Button type="button" variant="outline" onClick={() => onNavigate('prev')} disabled={activeLessonIndex <= 0}>
                        Bài trước
                    </Button>
                    <Button type="button" variant="outline" onClick={() => onNavigate('next')} disabled={activeLessonIndex < 0 || activeLessonIndex >= totalLessons - 1}>
                        Bài tiếp theo
                    </Button>
                </div>
    ...
    return (
        <div className="rounded-2xl border border-gray-100 overflow-hidden">
            {isYouTube && youtubeVideoId ? (
                ...
                <video key={url} src={url} controls className="w-full aspect-video bg-black" controlsList="nodownload" preload="metadata">
