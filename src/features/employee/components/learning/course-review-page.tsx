...
import { BookOpen, ClipboardCheck, Trophy, Star, CheckCircle2, Loader2 } from 'lucide-react';
...
export function CourseReviewPage({ initialCourse }: { initialCourse: Course }) {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const [feedbackCheck, progressData] = await Promise.all([
        ...
        <div className="space-y-6 max-w-5xl mx-auto px-4 py-8">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#0F4C75] via-[#1B4F72] to-[#3282B8] p-8 text-white shadow-xl">
                <div className="absolute inset-0 opacity-10" style={{
                    backgroundImage: 'radial-gradient(circle at 20% 50%, rgba(255,255,255,0.2) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(187,225,250,0.15) 0%, transparent 40%)',
                }} />
                <div className="relative z-10">
                    <p className="text-[11px] font-bold uppercase tracking-[3px] text-[#BBE1FA]/70 mb-2">Đánh giá khóa học</p>
                    <h1 className="text-2xl md:text-3xl font-black tracking-tight">{initialCourse.courseName}</h1>
                    <p className="text-sm text-white/60 mt-2">Mã khóa: {initialCourse.courseCode}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-4">
                    ...
                    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                        <p className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400 mb-4">Lộ trình học</p>
                        <div className="space-y-1">
                            {STEPS.map((step, idx) => {
                                const isActive = idx === 3; // Current step: Đánh giá
                                ...
                                return (
                                    <div key={step.label} className="flex items-center gap-3 relative">
                                        {/* Vertical line */}
                                        {idx < STEPS.length - 1 && (
                                            <div className={`absolute left-[15px] top-[32px] w-[2px] h-[20px] ${isCompleted ? 'bg-green-400' : 'bg-gray-200'}`} />
                                        )}
                                        {/* Circle */}
                                        <div className={`w-[30px] h-[30px] rounded-full flex items-center justify-center shrink-0 transition-all ${
                                            isCompleted ? 'bg-green-100 text-green-600'
                                            : isActive ? 'bg-[#0F4C75] text-white shadow-md'
                                        ...
                                        <span className={`text-sm font-medium ${
                                            isActive ? 'text-[#0F4C75] font-bold' : isCompleted ? 'text-green-700' : 'text-gray-400'
                                        }`}>{step.label}</span>
                                    </div>
                                );
                            })}
                    ...
                    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm space-y-3">
                        <p className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400">Thông tin khóa học</p>
                        <div className="text-sm text-gray-600 space-y-2">
                            <div className="flex justify-between">
                                <span className="text-gray-400">Mã khóa</span>
                                <span className="font-semibold text-[#0F4C75]">{initialCourse.courseCode}</span>
                            </div>
                            {totalLessons > 0 && (
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Bài học</span>
                                    <span className="font-semibold text-[#0F4C75]">{completedLessons}/{totalLessons}</span>
                                </div>
                            )}
                            <div className="flex justify-between">
                                <span className="text-gray-400">Tiến độ</span>
                                <span className="font-semibold text-green-600">{progress?.progressPercentage ?? 0}%</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="rounded-2xl border border-gray-100 bg-white p-6 md:p-8 shadow-sm">
                        {isChecking ? (
                            <div className="flex items-center justify-center py-16">
                                ...
                                <span className="ml-2 text-sm text-gray-500">Đang kiểm tra...</span>
                            </div>
                        ) : hasSubmitted ? (
                            ...
                            <div className="text-center py-12 space-y-4">
                                ...
                                <h3 className="text-xl font-black text-[#0F4C75]">Bạn đã đánh giá khóa học này</h3>
                                <p className="text-sm text-gray-500 max-w-md mx-auto">
                                    Cảm ơn bạn đã chia sẻ trải nghiệm! Đánh giá của bạn giúp cải thiện chất lượng đào tạo.
                                </p>
                                <button
                            ...
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-xl font-black text-[#0F4C75]">Chia sẻ trải nghiệm của bạn</h2>
                                    <p className="text-sm text-gray-500 mt-1">Đánh giá giúp chúng tôi cải thiện chất lượng đào tạo.</p>
                                </div>
                                <CourseFeedbackForm
                                    courseId={initialCourse.id}
