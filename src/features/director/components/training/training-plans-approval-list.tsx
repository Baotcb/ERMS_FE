...
import {
  STATUS_COLORS,
  getStatusLabel,
} from "../../../hr/utils/training-status-utils";
...
export function TrainingPlansApprovalList({
  ...
  const {
    ...
    fetcher: (params) =>
      directorTrainingService.getPlans({ ...params, status: statusFilter }),
    initialData,
  ...
  const handleApprove = () =>
    execute(() => directorTrainingService.approvePlan(selectedPlan!.id), {
      successMessage: {
        title: "Đã phê duyệt",
        description: "Kế hoạch đào tạo đã được phê duyệt thành công.",
      },
      onSuccess: () => {
  ...
  const handleRequestResubmission = () => {
    ...
    return execute(
      () =>
        directorTrainingService.requestPlanResubmission(
          selectedPlan!.id,
          resubmitRequestNote.trim(),
        ),
      {
        successMessage: {
          title: "Đã yêu cầu gửi lại",
          description: "Kế hoạch đã được trả về HR để chỉnh sửa và gửi lại.",
        },
        onSuccess: () => {
  ...
  const handleReject = () => {
    ...
    return execute(
      () =>
        directorTrainingService.rejectPlan(
          selectedPlan!.id,
          rejectReason.trim(),
        ),
      {
        successMessage: {
          title: "Đã từ chối",
          description: "Kế hoạch đào tạo đã bị từ chối.",
        },
        onSuccess: () => {
  ...
  const handleClosePlan = () =>
    execute(
      () =>
        directorTrainingService.closePlan(
          selectedPlan!.id,
  ...
  return (
    <div className="space-y-6">
          ...
          {(["Pending", "Approved", "Closed"] as const).map((status) => {
            ...
            return (
              <Button
                ...
                className={
                  isActive
                    ? "bg-[#0F4C75] hover:bg-[#1A5F8C] text-white"
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-[#0F4C75] border-blue-200 hover:bg-blue-50"
                          onClick={() => {
      ...
      <Dialog open={isApproveOpen} onOpenChange={setIsApproveOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn phê duyệt kế hoạch{" "}
      ...
      <Dialog
        open={isResubmitRequestOpen}
        onOpenChange={setIsResubmitRequestOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Yêu cầu gửi lại</DialogTitle>
            <DialogDescription>
              Ghi chú cho HR về nội dung cần chỉnh sửa trong kế hoạch{" "}
              <strong>{selectedPlan?.planName}</strong>.
            </DialogDescription>
          </DialogHeader>
          <Textarea
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSubmitting || !resubmitRequestNote.trim()}
