...
function PendingPlanRow({ plan }: { plan: RecruitmentPlan }) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      onClick={() =>
        router.push(`/enterprise/director/recruitment-plans/${plan.id}`)
      }
      onKeyDown={(e) => {
...
function StatCard({
  ...
  return (
    <div
      className={`p-4 rounded-lg bg-white border border-l-4 shadow-sm ${color}`}
    >
...
export const DirectorDashboard = memo(function DirectorDashboard() {
  ...
  const { data: pendingPlansData } = useSWR<PlanListResponse>(
    ...
    () =>
      apiClient
        .get("/api/RecruitmentPlans?Status=Pending&Page=1&PageSize=5")
        .then((res) => res.json()),
  );
...
<<<<<<< HEAD
  ...
  const { data: trainingBudgetData } = useSWR(
    ...
    () =>
      apiClient
        .get("/api/TrainingPlans/budget-summary")
        .then((res) => res.json())
        .catch(() => []), // Fallback to empty array if endpoint missing
  );
  ...
  const activities: ActivityItem[] = [
    {
      id: "1",
      title: "HR Manager đã duyệt danh sách ứng viên Marketing",
      time: "2 giờ trước",
      type: "success",
    },
    {
      id: "2",
      title: "Cần phê duyệt ngân sách điều chỉnh cho IT Dept",
      time: "5 giờ trước",
      type: "warning",
    },
    {
      id: "3",
      title: "Báo cáo tuyển dụng Q1/2026 đã sẵn sàng",
      time: "1 ngày trước",
      type: "info",
    },
  ];
...
>>>>>>> 45aae32ad9c33ecdcf340333462dab586d35d0e3
  ...
  return (
    <div className="flex flex-col gap-6 min-h-[calc(100vh-6rem)]">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#0F4C75] leading-tight">
          Dashboard
        </h1>
        ...
        <StatCard
          ...
          value={
            isLoadingJobs ? (
              ...
              activeJobsData?.totalCount?.toString() || "0"
            )
          }
...
>>>>>>> 45aae32ad9c33ecdcf340333462dab586d35d0e3
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
        {budgetChartData.length > 0 ? (
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-center items-center min-h-[300px]">
            <h4 className="absolute top-6 left-6 text-base font-bold text-gray-800">
              Phân bổ ngân sách đào tạo
            </h4>
            <p className="text-gray-400 mt-10">Chưa có dữ liệu</p>
          </div>
        )}
